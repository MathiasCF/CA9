...
sigma = 4;%2.5;% + 0.5*ep;                                            %width of features
...
demand = demand.dcon*5.77+5;     %5.75
%demand = demand*0 + 2;
...
S0 = [h/2 1]';       % set initial state S0(h,t)
 ...
 testheight = 28;
 for f = 1:weights
            feature27(f) = exp((-(abs(testheight-centers(f)))^2)/(2*sigma^2));
 end
 tt = 4.5*(ep);
         tfeature1(1) = 1;
         tfeature1(2) = sin(2*pi*f0*tt);
         tfeature1(3) = cos(2*pi*f0*tt);
%         
        tfeature1(4) = sin(4*pi*f0*tt);
        tfeature1(5) = cos(4*pi*f0*tt);
% 
% 
%         
          tfeature1(6) = sin(6*pi*f0*tt);
          tfeature1(7) = cos(6*pi*f0*tt);
%         
          tfeature1(8) = sin(8*pi*f0*tt);
          tfeature1(9) = cos(8*pi*f0*tt);
% 
          tfeature1(10) = sin(12*pi*f0*tt);
          tfeature1(11) = cos(12*pi*f0*tt);
...
for episode = 1:1
    ...
    for ii = 1:maxstep
        ...
        newstate = nextstatecontinioustimeRealHeight(state, Q, epsilon,h,demand,timediscretisation);    %gives new height and next time, and action
...
% 
        ...
        Q1(ep,ii) = w(1,:)*[feature27 tfeature1]';
        Q2(ep,ii) = w(2,:)*[feature27 tfeature1]';      
        Q3(ep,ii) = w(3,:)*[feature27 tfeature1]';
...
figure(16)
for i = 1:ep
plot(Q1(i,:)', 'Color', ColorSet(i,:))
hold on
plot(Q2(i,:)', 'Color', ColorSet(i,:))
plot(Q3(i,:)', 'Color', ColorSet(i,:))
